...
export default class ResultTabView extends View {
  ...
  declare public selectedResult: string | null;
  ...
  constructor(locale: Locale, lookupResults: MerriamWebsterResult[]) {
    ...
    lookupResults.forEach((result, index) => {
      ...
      selectTabButton.set({
        label: `${index + 1}. ${result.label}`,
        withText: true,
      });
      selectTabButton.bind('class')
        .to(this, 'selectedResult', value => value === viewUid ?
          'ck ck-sub-tab ck-selected' : 'ck ck-sub-tab'
        );
      ...
      resultBlock.setTemplate({
        ...
        attributes: {
          class: [
            'ck',
            bind.if( 'selectedResult', 'ck-hidden', value => value !== viewUid )
          ]
        },
